...
def load_images():
    ...
    for piece in pieces:
        IMAGES [piece] = py.transform.scale(py.image.load("Assets/Images/" + piece + ".png"), (SQ_SIZE, SQ_SIZE))


def main():
    py.init()
    py.mixer.init()
    move_sound = py.mixer.Sound("Assets/Sounds/move-self.mp3")
    capture_sound = py.mixer.Sound("Assets/Sounds/capture.mp3")
    check_sound = py.mixer.Sound("Assets/Sounds/move-check.mp3")
    castle_sound = py.mixer.Sound("Assets/Sounds/castle.mp3")
    game_over_sound = py.mixer.Sound("Assets/Sounds/game-end.mp3")

    
    game_clock = ChessClock.ChessClock(300)  # 3-minute timer
